...
- **Live Threat Telemetry**: Real-time "COMPETITOR ACTIVITY" badge in header (navy glassmorphism design, absolute center positioned). Stricter thresholds: Score <30 = "Low 🟢" (green glow), Score 30-65 = "Moderate 👀" (yellow/orange glow), Score >65 = "HIGH 🔥" (red pulse). War Room sidebar button pulses red (high) or glows yellow (moderate). Data sourced from `intelligence_scores` view with fallback to `website_data` activity density.
