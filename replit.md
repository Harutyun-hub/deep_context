...
- **Competitive Intelligence Suite**: `dashboard.html` with client-vs-competitor comparison using Supabase queries and Chart.js. Dashboard queries `facebook_ads`, `google_ads`, and `instagram_posts` tables directly with proper foreign key joins to `companies` table via `company_id`. Date filtering uses `snapshot_date` field on all tables. Default date range is set to last 7 days on page load. Company filter uses `company_id` (uuid) for precise filtering.
